...
import { Upload, FileText, Save, Search } from 'lucide-react';
...
const OrderList = () => {
  ...
  const imageGalleryRef = useRef<ImageGalleryHandle>(null);  const [excelDataSets, setExcelDataSets] = useState<ExcelData[]>(initialExcelData);
  ...
  const [errorMessage, setErrorMessage] = useState<string>('');  const [hidePanelColumns, setHidePanelColumns] = useState(false);  const [successMessage, setSuccessMessage] = useState('');
  ...
  // Save project handler with enhanced Order No handling
  const handleSaveProject = async () => {
    ...
    try {
      // Check if a project with this Order No already exists
      const existingProject = await projectService.findProjectByOrderNo(orderNo);
      
      let confirmSave = true;
      if (existingProject) {
        confirmSave = window.confirm(
          `A project with Order No "${orderNo}" already exists. ` +
          'Do you want to update it with the current data?'
        );
      }
      ...
      if (saved) {
        const action = existingProject ? 'updated' : 'saved';
      ...
      setErrorMessage(`Error saving project: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const tableRef = useRef<HTMLTableElement>(null);  // New state for drag and drop functionality
  ...
  const extractSpecificCells = (workbook: XLSX.WorkBook) => {
    try {
      ...
      let cellB2Value = '';
          if (cellA2 && cellA2.v) {
        let rawA2 = String(cellA2.v);
        cellA2Value = rawA2;
        // A2 contains the Order No - try to extract it if it has a pattern like 'Order No: ORDERNO'
        const orderMatch = rawA2.match(/Order\s*No[:\s]*(.+)/i);
        if (orderMatch && orderMatch[1]) {
          orderNoValue = orderMatch[1].trim();
        } else {
          orderNoValue = rawA2.trim();
        }
        console.log('Extracted orderNo from cell A2:', orderNoValue);
      }

      if (cellB2 && cellB2.v) {
        let rawB2 = String(cellB2.v);
        cellB2Value = rawB2;
        // B2 contains the Shop Order Note - try to extract it if it has a pattern like 'Shop Order Note: VALUE'
        const shopMatch = rawB2.match(/Shop\s*Order\s*Note[:\s]*(.+)/i);
        if (shopMatch && shopMatch[1]) {
          shopOrderValue = shopMatch[1].trim();
        } else {
          shopOrderValue = rawB2.trim();
        }
        console.log('Extracted shopOrderNote from cell B2:', shopOrderValue);
      }
            ...
            if (index === 0) {
              ...
              if (!orderNo || !shopOrderNote) {
                ...
                if (!shopOrderNote) {
                  ...
                  } else {                    // Look for cell A1 for potential "Order No" label
                    const cellA1 = worksheet['A1'];
                    if (cellA1 && cellA1.v && String(cellA1.v).includes("Order No")) {
                      console.log('Found "Order No" label in A1, using A2 value as order no');
                      const cellA2 = worksheet['A2'];
                      if (cellA2 && cellA2.v) {
                        const a2Value = String(cellA2.v).trim();
                        setOrderNo(a2Value);
                      }
                    }
                    
                    // Look for cell B1 for potential "Shop Order Note" label  
                    const cellB1 = worksheet['B1'];
                    if (cellB1 && cellB1.v && String(cellB1.v).includes("Shop Order Note")) {
                      console.log('Found "Shop Order Note" label in B1, using B2 value as shop order note');
  ...
  const createMergedData = useCallback((datasets: ExcelData[]) => {
    ...
    console.log('Merged data created with', newMergedData.length, 'rows (including header)');
  ...
  const handleExportPDF = () => {
    exportOrderDataAsHTML(
      ...
      highlightedCells // Add highlighted cells to export function
    );
  };
        ...
        const targetMaterialKey = `material-${dropTarget.datasetIndex}-${dropTarget.rowIndex}`;
          ...
          if (cell) {
            gsap.to(cell, {
              ...
              boxShadow: 'inset 0 0 0 2px rgba(59, 130, 246, 0.4)',
              onComplete: () => {
                // Check if this cell should remain highlighted after animation
                const isHighlighted = highlightedCells.has(key);
                gsap.to(cell, { 
                  duration: 0.3, 
                  background: isHighlighted ? 'rgba(156, 163, 175, 0.3)' : 'transparent', 
                  boxShadow: 'none' 
                });
              }
            });
  ...
  const resetDragState = () => {
    // Reset all cells, but preserve highlighting
    Object.keys(cellRefs.current).forEach(key => {
      const cell = cellRefs.current[key];
      if (cell) {
        // Check if this cell should remain highlighted
        const isHighlighted = highlightedCells.has(key);
        gsap.to(cell, {
          duration: 0.2,
          background: isHighlighted ? 'rgba(156, 163, 175, 0.3)' : 'transparent',
  ...
  const handleCellClick = (datasetIndex: number, rowIndex: number, cellType: 'panel' | 'material') => {
    const cellKey = `${cellType}-${datasetIndex}-${rowIndex}`;
    ...
    if (highlightedCells.has(cellKey)) {
      ...
      if (cell) {
        gsap.to(cell, {
          duration: 0.2,
          background: 'rgba(156, 163, 175, 0.3)', // Light gray highlight
          ease: "power1.out"
        });
      }
    }
  ...
  const filteredExcelDataSets = excelDataSets.map(dataset => ({
    ...dataset,
    rows: dataset.rows.filter(row => {
      if (!searchTerm.trim()) return true;
      const searchLower = searchTerm.toLowerCase();
      return (
        row.panelNo.toLowerCase().includes(searchLower) ||
        row.material.toLowerCase().includes(searchLower)
      );
    })
  }));

  // Calculate filtered max row count
  const filteredMaxRowCount = Math.max(...filteredExcelDataSets.map(ds => ds.rows.length), 0);
        ...
        <div className="action-bar">
          ...
          <div className="search-container">
            <div className="search-input-wrapper">
              ...
              <input
                type="text"
                placeholder="Search panel or material..."
                value={searchTerm}
            ...
            {highlightedCells.size > 0 && (
              <button
                onClick={() => {
                  // Clear all highlights with animation
            ...
            <div className="table-area">
              <div className="table-container">
                <table ref={tableRef} className="modern-table data-table">
                  <thead>
                    <tr>
                      {filteredExcelDataSets.map((dataset, datasetIdx) => (
                  ...
                  <tbody>
                    {Array.from({ length: filteredMaxRowCount }).map((_, rowIdx) => (
                          ...
                          <React.Fragment key={`${dataset.id}-${rowIdx}`}>
                            {dataset.rows[rowIdx] ? (
                              <>                                <td 
                                  ...
                                  className={`table-cell panel-cell ${hidePanelColumns && datasetIdx > 0 ? 'hidden' : ''} ${highlightedCells.has(`panel-${datasetIdx}-${rowIdx}`) ? 'highlighted' : ''}`}
                                  draggable={true}
                                ...
                                <td 
                                  ...
                                  className={`table-cell material-cell ${highlightedCells.has(`material-${datasetIdx}-${rowIdx}`) ? 'highlighted' : ''}`}
                                  draggable={true}
