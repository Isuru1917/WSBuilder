...
export const exportOrderDataAsHTML = (
  ...
  const visibleDataRows = dataRows.slice(0, Math.min(dataRows.length, rowsPerPage));
  ...
  printWindow.document.write(`
    ...
    <head>
      ...
      <style>
        ...
        .webbing-section, .webbing-table {
          transition: opacity 0.3s ease-out, height 0.3s ease-out;
        }
        .webbing-section.hidden, .webbing-table.hidden {
          display: none;
        }
        .timestamp {
          font-size: 9px;
      ...
      <div class="container">
        <div class="print-controls no-print">
          <div class="button-group">
            ...
            <button class="action-button" id="toggleWebbingBtn">Hide Webbing</button>
          </div>
        </div>
  ...
  if (webbingDataRows && webbingDataRows.length > 0) {
    printWindow.document.write(`
      <div class="webbing-section" style="margin-top: 30px; margin-bottom: 10px;">
  ...
  printWindow.document.write(`
      ...
      <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
          ...
          // Add functionality for the webbing toggle button
          const toggleWebbingBtn = document.getElementById("toggleWebbingBtn");
          if (toggleWebbingBtn) {
            toggleWebbingBtn.addEventListener("click", function() {
              // Select both the webbing heading section and the webbing table
              const webbingSection = document.querySelector(".webbing-section");
              const webbingTable = document.querySelector(".webbing-table");
              
              // Toggle both elements' visibility
              if (webbingSection) {
                webbingSection.classList.toggle("hidden");
              }
              if (webbingTable) {
                webbingTable.classList.toggle("hidden");
              }
              
              this.textContent = this.textContent === "Hide Webbing" ? "Show Webbing" : "Hide Webbing";
            });
          }
          
          // Automatically apply fit-to-page functionality on page load
          // Hide all content after the first page
          const pageBreaks = document.querySelectorAll('[style*="page-break-before"]');
          pageBreaks.forEach(pageBreak => {
            pageBreak.style.display = "none";
          });
          
          // Reduce font size and adjust spacing to make content fit
          const mainTable = document.querySelector("table:not(.webbing-table)");
          if (mainTable) {
